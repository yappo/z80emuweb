...
`DD` / `FD` は、後続命令中の `HL` / `H` / `L` / `(HL)` を単純な
文字列置換で変換するものではない。オペランドの種類によって解釈が異なる。

| 後続命令中の形式 | `DD` | `FD` |
|---|---|---|
| 16bit レジスタ対 `HL` | `IX` | `IY` |
| 8bit レジスタ `H` | `IXH` | `IYH` |
| 8bit レジスタ `L` | `IXL` | `IYL` |
| メモリオペランド `(HL)` | `(IX+d)` | `(IY+d)` |

ただし、`LD r,(IX+d)` および `LD (IX+d),r` の `r` に含まれる `H` / `L` は
通常の `H` / `L` であり、`IXH` / `IXL` には変換しない。`IY` についても同じである。

例えば、`DD 66 d` は `LD H,(IX+d)` であり、`LD IXH,(IX+d)` ではない。
この命令で変化するのは `H` であって `IX` ではない。
