...
| バイト列 | 命令 | `H` / `L` の解釈 |
|---|---|---|
| `DD 66 d` | `LD H,(IX+d)` | 通常の `H` |
| `DD 6E d` | `LD L,(IX+d)` | 通常の `L` |
| `DD 74 d` | `LD (IX+d),H` | 通常の `H` をメモリへ書込 |
| `DD 75 d` | `LD (IX+d),L` | 通常の `L` をメモリへ書込 |
| `DD 64` | `LD IXH,IXH` | `IXH` |
| `DD 65` | `LD IXH,IXL` | `IXH` / `IXL` |

特に `DD 66 02` は `LD H,(IX+2)` であり、`LD IXH,(IX+2)` ではない。
