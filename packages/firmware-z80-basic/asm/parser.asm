...
; ----------------------------------------------------------------------------
; ルーチン: SET_SYNTAX_ERROR
; 役割: 汎用構文エラーを RAM_LAST_ERROR へ設定する。
; ----------------------------------------------------------------------------
SET_SYNTAX_ERROR:
  LD A,1
  LD (RAM_LAST_ERROR),A
...
; ----------------------------------------------------------------------------
; ルーチン: ASSERT_END_FROM_HL
; 役割: HL 以降が空白と終端のみであることを検証する。
;       余剰トークンがある場合は構文エラーを設定する。
; ----------------------------------------------------------------------------
ASSERT_END_FROM_HL:
  CALL SKIP_SPACES
  LD A,(HL)
...
; ----------------------------------------------------------------------------
; ルーチン: ASSERT_END_AFTER_CMD
; 役割: コマンド直後引数に余剰トークンがないことを検証する。
; ----------------------------------------------------------------------------
ASSERT_END_AFTER_CMD:
  CALL LOAD_AFTER_CMD_HL
  JP ASSERT_END_FROM_HL

; ----------------------------------------------------------------------------
; ルーチン: ASSERT_END_FROM_EXPRPTR
; 役割: 式パーサ位置以降に余剰トークンがないことを検証する。
; ----------------------------------------------------------------------------
ASSERT_END_FROM_EXPRPTR:
  CALL EXPR_GET_PTR_HL
  JP ASSERT_END_FROM_HL
