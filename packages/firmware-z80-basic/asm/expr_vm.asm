...
PARSE_FACTOR_VARIABLE_OK:
  ...
  LD A,H
  LD (RAM_TMP_VAL_HI),A
  ; A(1) 形式は添字を無視してスカラー変数として扱う。
  CALL EXPR_SKIP_SPACES
  CALL EXPR_GET_PTR_HL
  LD A,(HL)
  CP '('
  JR NZ,PARSE_FACTOR_VARIABLE_RESTORE
  INC HL
  CALL EXPR_SET_PTR_HL
  CALL EVAL_EXPRESSION
  CALL EXPR_SKIP_SPACES
...
SET_VARIABLE_FROM_DE:
  ...
  LD A,(RAM_TMP_KEY3)
  LD (HL),A
  ; 新規割当時は HL が key3 を指しているため、
  ; エントリ先頭(key1)へ戻して既存経路と同一の保存処理に揃える。
  DEC HL
  DEC HL
  JR SET_VARIABLE_HAVE_ENTRY

SET_VARIABLE_HAVE_ENTRY:
  LD A,1
  LD (RAM_VAR_CACHE_VALID),A
  LD A,(RAM_TMP_KEY1)
  LD (RAM_VAR_CACHE_KEY1),A
  LD A,(RAM_TMP_KEY2)
  LD (RAM_VAR_CACHE_KEY2),A
  LD A,(RAM_TMP_KEY3)
  LD (RAM_VAR_CACHE_KEY3),A
  LD A,L
  LD (RAM_VAR_CACHE_PTR_LO),A
  LD A,H
  LD (RAM_VAR_CACHE_PTR_HI),A
  INC HL
  INC HL
  INC HL
...
; ----------------------------------------------------------------------------
FIND_VARIABLE_ENTRY:
  LD A,(RAM_VAR_CACHE_VALID)
  OR A
  JR Z,FIND_VARIABLE_ENTRY_SCAN
  LD A,(RAM_VAR_CACHE_KEY1)
  LD C,A
  LD A,(RAM_TMP_KEY1)
  CP C
  JR NZ,FIND_VARIABLE_ENTRY_SCAN
  LD A,(RAM_VAR_CACHE_KEY2)
  LD C,A
  LD A,(RAM_TMP_KEY2)
  CP C
  JR NZ,FIND_VARIABLE_ENTRY_SCAN
  LD A,(RAM_VAR_CACHE_KEY3)
  LD C,A
  LD A,(RAM_TMP_KEY3)
  CP C
  JR NZ,FIND_VARIABLE_ENTRY_SCAN
  LD A,(RAM_VAR_CACHE_PTR_LO)
  LD L,A
  LD A,(RAM_VAR_CACHE_PTR_HI)
  LD H,A
  SCF
  RET

FIND_VARIABLE_ENTRY_SCAN:
  LD HL,RAM_VAR_TABLE
